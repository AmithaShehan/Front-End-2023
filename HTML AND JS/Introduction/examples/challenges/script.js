...
function showMeme() {
  ...
  const isImg = memeContainer.querySelector('img');
  clearAll();
...
function showQuote() {
  ...
  const quote1 = randomQuote.quote;
  const quoteBy = randomQuote.author;
  const fullQuote = quote1 + ' - ' + quoteBy;
  newQuote.textContent = fullQuote;
  clearAll();
...
function showRiddle() {
  ...
  answerContainer.textContent = 'The answer is '  + randomRiddle.answer;
